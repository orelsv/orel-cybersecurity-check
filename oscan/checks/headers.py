...
def check_security_headers(ctx: ScanContext) -> list[Finding]:
    ...
    def need(header, fid, title, severity, why, fix, refs=None):
        if header in h:
            findings.append(passed(fid, f"{title} present", CATEGORY, evidence=h[header][:120], location=url))
        else:
            findings.append(Finding(
                id=fid, title=f"Missing {title}", severity=severity, category=CATEGORY,
                location=url, why=why, fix=fix, references=refs or [],
            ))

    is_https = url.startswith("https://")
    if is_https:
        need(
            "strict-transport-security", "HDR-001", "HSTS (Strict-Transport-Security)",
            Severity.MEDIUM,
    ...
    need(
        "x-content-type-options", "HDR-002", "X-Content-Type-Options",
        Severity.LOW,
    ...
    if "x-frame-options" in h or "frame-ancestors" in csp.lower():
        findings.append(passed("HDR-003", "Clickjacking protection present", CATEGORY, location=url))
    else:
        findings.append(Finding(
            id="HDR-003", title="No clickjacking protection (X-Frame-Options / CSP frame-ancestors)",
    ...
    need(
        "content-security-policy", "HDR-004", "Content-Security-Policy",
        Severity.MEDIUM,
    ...
    need(
        "referrer-policy", "HDR-005", "Referrer-Policy",
        Severity.LOW,
    ...
    if disclosed:
        findings.append(Finding(
            id="HDR-006", title="Software version disclosure",
            severity=Severity.LOW, category=CATEGORY, location=url,
