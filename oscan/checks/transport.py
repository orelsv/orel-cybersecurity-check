...
def check_tls(ctx: ScanContext) -> list[Finding]:
    ...
    if parsed.scheme != "https":
        findings.append(Finding(
            id="TLS-000", title="Site served over plaintext HTTP",
    ...
    if info.error:
        findings.append(Finding(
            id="TLS-001", title="TLS connection / certificate problem",
            severity=Severity.HIGH, category=CATEGORY, location=url,
            evidence=info.error,
    ...
    if info.version in _WEAK_TLS:
        findings.append(Finding(
            id="TLS-002", title=f"Weak TLS protocol negotiated ({info.version})",
            severity=Severity.HIGH, category=CATEGORY, location=url,
        ...
        findings.append(passed("TLS-002", f"Modern TLS in use ({info.version})", CATEGORY, location=url))
    ...
    if info.not_after:
        try:
            ...
            if days < 0:
                findings.append(Finding(
                    id="TLS-003", title="TLS certificate expired",
                    severity=Severity.HIGH, category=CATEGORY, location=url,
                ...
                findings.append(Finding(
                    id="TLS-004", title="TLS certificate expiring soon",
...
def check_http_redirect(ctx: ScanContext) -> list[Finding]:
    ...
    return [Finding(
        id="TLS-005", title="HTTP does not redirect to HTTPS",
        severity=Severity.MEDIUM, category=CATEGORY, location=http_url,
