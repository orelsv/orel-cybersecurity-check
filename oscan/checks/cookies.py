...
_TOKEN_KEYS = {"token", "access_token", "id_token", "auth", "apikey", "api_key",
               "session", "sessionid", "sid", "jwt", "key", "secret", "password"}
...
def _parse_set_cookie(raw: str) -> dict:
    ...
    return {"name": name, "secure": "secure" in attrs,
            "httponly": "httponly" in attrs, "samesite": samesite,
...
def check_cookies(ctx: ScanContext) -> list[Finding]:
    ...
    if not raw_cookies:
        findings.append(passed("COOKIE-000", "No cookies set on the home page", CATEGORY, location=url))
    ...
    for raw in raw_cookies:
        ...
        if is_https and not c["secure"]:
            findings.append(Finding(
                id="COOKIE-001", title=f"Cookie '{c['name']}' missing Secure flag",
                severity=Severity.HIGH, category=CATEGORY, location=loc,
        ...
        if not c["httponly"]:
            findings.append(Finding(
                id="COOKIE-002", title=f"Cookie '{c['name']}' missing HttpOnly flag",
                severity=Severity.MEDIUM, category=CATEGORY, location=loc,
        ...
        if c["samesite"] in ("", "none"):
            findings.append(Finding(
                id="COOKIE-003", title=f"Cookie '{c['name']}' has weak SameSite ({c['samesite'] or 'unset'})",
                severity=Severity.LOW, category=CATEGORY, location=loc,
        ...
        if _JWT_RE.search(c["value"]):
            findings.append(Finding(
                id="COOKIE-004", title=f"JWT stored in cookie '{c['name']}'",
                severity=Severity.INFO, category=CATEGORY, location=loc,
    ...
    for candidate in {ctx.target.url, url}:
        ...
        if leaked or _JWT_RE.search(candidate):
            findings.append(Finding(
                id="TOKEN-001", title="Token or credential present in URL",
                severity=Severity.HIGH, category=CATEGORY, location=candidate.split("?")[0],
