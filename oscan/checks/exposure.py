...
_PROBES = [
    (
        ".git/HEAD",
        lambda t: t.strip().startswith("ref:"),
        "EXP-001",
    ...
    (
        ".env",
        lambda t: not _looks_html(t) and re.search(r"^[A-Z0-9_]+=", t, re.M) is not None,
        "EXP-002",
        "Exposed .env file",
        Severity.CRITICAL,
    ...
    (
        ...
        lambda t: (
            not _looks_html(t)
            and ("password" in t.lower() or "secret" in t.lower() or "apikey" in t.lower())
        ),
        "EXP-003",
        "Exposed config file with secrets",
        Severity.HIGH,
...
def check_exposure(ctx: ScanContext) -> list[Finding]:
    ...
    if not any_hit and not findings:
        findings.append(
            passed(
                "EXP-001", "No common sensitive files exposed", CATEGORY, location=ctx.target.url
            )
        )
    return findings
