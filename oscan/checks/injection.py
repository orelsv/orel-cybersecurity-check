...
_METADATA_RE = re.compile(
    ...
    r"computeMetadata|meta-data/|access_key)",
    re.I,
)
...
def check_injection(ctx: ScanContext) -> list[Finding]:
    ...
    if not points:
        return [
            Finding(
                id="INJ-000",
                title="No parameterized endpoints found to test",
    ...
    for i, url in enumerate(points):
        ...
        for name in list(params.keys()):
            findings.extend(
                _probe_param(ctx, parsed, params, name, do_timing=(i < _TIMING_MAX_POINTS))
            )
    if not findings:
        findings.append(
            passed(
                "INJ-001",
                "No reflected XSS / SQL error signatures detected",
                CATEGORY,
                location=ctx.target.url,
            )
        )
...
def _probe_param(
    ctx: ScanContext, parsed, params: dict, name: str, do_timing: bool = False
) -> list[Finding]:
    findings: list[Finding] = []
    ...
    try:
        ...
        if f"{marker}'\"<b>" in body or f'{marker}"<b>' in body:
            findings.append(
                Finding(
                    id="INJ-XSS",
                    title=f"Reflected input not encoded (possible XSS) in '{name}'",
    ...
    try:
        ...
        if _SQL_ERRORS.search(body):
            findings.append(
                Finding(
                    id="INJ-SQL",
                    title=f"SQL error triggered by quote in '{name}' (possible SQL injection)",
    ...
    try:
        ...
        if _NOSQL_ERRORS.search(body):
            findings.append(
                Finding(
                    id="INJ-NOSQL",
                    title=f"NoSQL error triggered in '{name}' (possible NoSQL injection)",
                    severity=Severity.CRITICAL,
                    category=CATEGORY,
                    location=where,
    ...
    try:
        ...
        if re.search(r"root:.*:0:0:", body):
            findings.append(
                Finding(
                    id="INJ-LFI",
                    title=f"Path traversal in '{name}' (file disclosure)",
                    severity=Severity.CRITICAL,
                    category=CATEGORY,
                    location=where,
    ...
    if name.lower() in ("next", "url", "redirect", "return", "returnurl", "dest", "continue"):
        p = dict(params)
        p[name] = "https://oscan-redirect.example/"
        try:
        ...
        for probe in (
            ...
            if _METADATA_RE.search(body):
                findings.append(
                    Finding(
                        id="INJ-SSRF",
                        title=f"Server-Side Request Forgery via '{name}'",
                        severity=Severity.CRITICAL,
                        category=CATEGORY,
                        location=where,
...
def _probe_time_based(
    ctx: ScanContext, parsed, params: dict, name: str, where: str
) -> list[Finding]:
    ...
    for payload in _time_payloads(params[name], delay):
        ...
        if elapsed - baseline >= threshold:
            return [
                Finding(
                    id="INJ-SQL-TIME",
                    title=f"Time-based blind SQL injection in '{name}'",
                    severity=Severity.CRITICAL,
                    category=CATEGORY,
                    location=where,
