...
def check_gdpr(ctx: ScanContext) -> list[Finding]:
    ...
    if tracker_hits:
        ...
        findings.append(
            Finding(
                id="GDPR-002",
                title="Third-party trackers loaded",
                severity=sev,
                category=CATEGORY,
                location=final_url,
                evidence=", ".join(sorted(tracker_hits)),
                why=(
                    "Trackers send visitor data (incl. IP) to third parties; without prior consent this breaches GDPR."
                    if not has_consent
                    else "Trackers process personal data; ensure they only run after consent and are in your privacy policy."
                ),
    ...
    if google_fonts:
        findings.append(
            Finding(
                id="GDPR-003",
                title="Google Fonts loaded from Google servers",
                ...
                references=[
                    "https://www.heise.de/news/Datenschutz-Google-Fonts-Abmahnungen-7178459.html"
                ],
            )
        )
    ...
    if (raw_cookies or tracker_hits) and not has_consent:
        ...
        findings.append(
            passed("GDPR-004", "Consent mechanism detected", CATEGORY, location=final_url)
        )
    ...
    if not has_privacy:
        ...
        findings.append(
            passed("GDPR-005", "Privacy policy link present", CATEGORY, location=final_url)
        )
        ...
        for form in soup.find_all("form"):
            ...
            if {"password", "email", "tel"} & inputs:
                findings.append(
                    Finding(
                        id="GDPR-006",
                        title="Form collecting personal data over HTTP",
                        severity=Severity.HIGH,
                        category=CATEGORY,
                        location=final_url,
