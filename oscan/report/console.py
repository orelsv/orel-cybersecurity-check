...
_COLOR = {
    ...
    Severity.LOW: "cyan", Severity.INFO: "green",
}
_GRADE_COLOR = {"A": "bold green", "B": "green", "C": "yellow", "D": "orange1", "F": "bold red"}


def render(console: Console, target: str, profile: str, summary: ScanSummary, findings: List[Finding]) -> None:
    ...
    issues = sorted((f for f in findings if f.severity is not Severity.INFO), key=lambda f: f.severity.rank)
    if not issues:
        ...
        for f in issues:
            table.add_row(
                ...
                f.id, f.title, f.location or "",
            )
        console.print(table)

    passed = [f for f in findings if f.severity is Severity.INFO]
    if passed:
        console.print(f"[green]✓ {len(passed)} controls in place / observations[/green] (see full report for details)")
